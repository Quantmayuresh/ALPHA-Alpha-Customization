...
def generate_e_invoice_json(docname):
    doc = load_doc("Subcontracting", docname)

    sup_data = frappe.get_doc("Address", doc.company_address)
    buy_data = frappe.get_doc("Address", doc.customer_address)
    ...
    item_list = []
    id = 0
    for d in doc.get("items_subcontracting"):
        id+=1
        item_dict ={
            "SlNo": id,
            ...
            "PreTaxVal": 0,
            "AssAmt": 0,
            "GstRt": int(d.cgst_rate) + int(d.sgst_rate),                           
    ...
    json_data ={
        ...
        "SellerDtls": {
            ...
            "LglNm": doc.customer_address,             #Legal Name
